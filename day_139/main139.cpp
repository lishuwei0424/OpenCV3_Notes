/*
案例 – 识别0～9印刷体数字 – PartOne

基于二值图像分析， 通过提取字符的40个特征向量生成字符特征
其中对每个字符的ROI区域，计算4x5的网格，对每个网络计算像素个数，然后生成20个特征向量。在这个过程中充分考虑到浮点数Cell大小与分割，采用基于权重的分割方法，实现网格分割。
...
首先来看一下如何提取40个特征向量， 这个里面涉及到两个很关键的技巧，一个是浮点数权重、一个是归一化，浮点数权重是可以提升准确率，归一化在某些程度上是可以保证字符的放缩不变性。
*/
...
void test_data() {
	Mat src1 = imread("../images/digit-02.png");
	...
	for (size_t t = 0; t < contours.size(); t++) {
		...
		Mat roi = binary(rect);
		//bitwise_not(roi, roi);
		vector<float> vf = extractFeatureData(roi);
...
void train_data() {
	Mat src1 = imread("../images/td1.png");
	Mat gray, binary;
	cvtColor(src1, gray, COLOR_BGR2GRAY);
	threshold(gray, binary, 0, 255, THRESH_BINARY_INV | THRESH_OTSU);
	vector<vector<Point>> contours;
